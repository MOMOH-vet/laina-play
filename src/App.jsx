...
function AIExhibitionLanding() {
  ...
  useEffect(() => { const el = document.getElementById('jsonld'); if (el) el.textContent = JSON.stringify(eventJsonLd); }, [eventJsonLd]);

  const RSVP_ENDPOINT = null;
  ...
  return (
    <div className="min-h-screen relative">
      <script id="jsonld" type="application/ld+json" dangerouslySetInnerHTML={{ __html: JSON.stringify(eventJsonLd) }} />
        ...
        <main id="main" className="max-w-7xl mx-auto px-4 pt-14 md:pt-20 pb-14 md:pb-24">
          <div className="grid md:grid-cols-2 lg:grid-cols-[1.05fr_1fr] gap-10 md:gap-12 items-start">
            ...
            <div className="relative">
              <div className="aspect-[4/3] rounded-3xl ring-1 ring-white/10 overflow-hidden">
                <HeroVisual />
              </div>
              <p className="mt-3 text-xs text-slate-400">{t('hero.visual.caption')}</p>
            </div>
          </div>
        </main>
      </section>
  ...
  return (
    <div className="relative w-full h-full">
      <svg viewBox="0 0 800 600" className="w-full h-full" preserveAspectRatio="xMidYMid slice" aria-hidden="true">
        ...
        <g>
          <circle cx="200" cy="320" r="160" fill="url(#g1)" opacity="0.5" />
          <circle cx="560" cy="260" r="140" fill="url(#g1)" opacity="0.5" />
        </g>
        <g stroke="#ffffff" strokeOpacity="0.08">
          {Array.from({ length: 24 }).map((_, i) => (<line key={i} x1={i * 35} y1="0" x2={i * 35} y2="600" />))}
          {Array.from({ length: 16 }).map((_, i) => (<line key={i} x1="0" y1={i * 35} x2="800" y2={i * 35} />))}
        </g>
        <g>
          {Array.from({ length: 60 }).map((_, i) => {
            const x = 60 + i * 12;
            const y = 300 + Math.sin(i / 2) * 40;
            return <circle key={i} cx={x} cy={y} r={2} fill="#e2e8f0" opacity="0.7" />;
          })}
        </g>
      </svg>
    </div>
...
function BackgroundFX() {
  ...
  useEffect(() => {
    ...
    const init = () => {
      const count = Math.round(window.innerWidth * window.innerHeight / 15000);
      stars = Array.from({ length: count }, () => ({
        x: Math.random() * window.innerWidth,
        y: Math.random() * window.innerHeight,
        r: Math.random() * 1.2 + 0.2,
        p: Math.random() * Math.PI * 2,
        s: (Math.random() * 0.7 + 0.3) / 1000
      }));
    };
    ...
    const loop = (t) => {
      ...
      for (const star of stars) {
        const alpha = 0.12 + 0.25 * (0.5 + Math.sin(t * star.s + star.p));
  ...
  useEffect(() => {
    ...
    const onScroll = () => {
      ...
      glow.style.setProperty('--glowShift', ((-(y % 120) / 6).toFixed(1)) + 'px');
    };
  ...
  return (
    <div id="bgfx" aria-hidden="true">
      <canvas id="bg-stars" ref={starsRef} />
      <div id="glowfx" ref={glowRef} />
    </div>
  );
}
...
function CursorFX() {
  ...
  useEffect(() => {
    ...
    const TRAIL_PER_MOVE = 4;
    let rafId = null;
    let lastTime = performance.now();
    let lastClickable = null;

    const spawn = (x, y, speed, angle, size, life) => {
      particles.push({
        x,
        y,
    ...
    const loop = (timestamp) => {
      ...
      for (let i = particles.length - 1; i >= 0; i -= 1) {
        ...
        const radius = particle.size * (0.5 + 0.5 * t);
        ...
        ctx.globalAlpha = 0.9 * Math.max(0.2, t);
        ctx.beginPath();
        ctx.arc(particle.x, particle.y, radius * 0.6, 0, Math.PI * 2);
        ctx.fill();
        ctx.globalAlpha = 1;
      }
    ...
    const onPointerMove = (event) => {
      ...
      for (let i = 0; i < TRAIL_PER_MOVE; i += 1) {
        const angle = Math.random() * Math.PI * 2;
        const speed = 0.4 + Math.random() * 1.2;
        spawn(x, y, speed, angle, 1.5 + Math.random() * 1.2, 500 + Math.random() * 500);
      }
      ensureLoop();
    };
    ...
    const onPointerOver = (event) => {
      const clickable = closestClickable(event.target) || (getComputedStyle(event.target).cursor === 'pointer' ? event.target : null);
      if (clickable && clickable !== lastClickable) {
        ...
        for (let i = 0; i < count; i += 1) {
          ...
          spawn(x, y, speed, angle, 2 + Math.random() * 2, 600 + Math.random() * 500);
        }
        ensureLoop();
      } else if (!clickable) {
        lastClickable = null;
    ...
    const onPointerOut = (event) => {
      if (closestClickable(event.target) && !closestClickable(event.relatedTarget)) {
        lastClickable = null;
      }
    };
  ...
  return <canvas id="cursorfx-canvas" ref={canvasRef} />;
}
