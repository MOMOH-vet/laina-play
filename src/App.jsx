...
function AIExhibitionLanding() {
  ...
  useEffect(() => {
    ...
    if (el) el.textContent = JSON.stringify(eventJsonLd);
    return undefined;
  }, [eventJsonLd]);
...
function HeroVideo() {
  ...
  return (
    <div className="relative">
      ...
      <p className="mt-3 text-xs text-slate-400">Визуализация: видео — демонстрация</p>
    </div>
  );
}
...
function useBackgroundEffects({ starsRef, glowRef, cursorRef }) {
  useEffect(() => {
  ...
  useEffect(() => {
    ...
    const handleScroll = () => {
      ...
      element.style.setProperty('--glowShift', `${(-(y % 120) / 6).toFixed(1)}px`);
    };
  ...
  useEffect(() => {
    ...
    const handlePointerOver = (event) => {
      const computedCursor =
        typeof window !== 'undefined' && typeof window.getComputedStyle === 'function'
          ...
          : '';
      const clickable =
        closestClickable(event.target) ||
        (computedCursor === 'pointer' ? event.target : null);
    ...
    const loop = (timestamp) => {
      ...
      for (let i = particles.length - 1; i >= 0; i -= 1) {
        ...
        const r = particle.size * (0.5 + 0.5 * t);
        const gradient = ctx.createRadialGradient(particle.x, particle.y, 0, particle.x, particle.y, r * 3);
        ...
        ctx.beginPath();
        ctx.arc(particle.x, particle.y, r * 0.6, 0, Math.PI * 2);
        ctx.fill();
        ctx.globalAlpha = 1;
      }
...
export default function App() {
  ...
  useBackgroundEffects({ starsRef, glowRef, cursorRef });
  return (
    <>
      ...
      <canvas id="cursorfx-canvas" ref={cursorRef} />
      <AIExhibitionLanding />
    </>
  );
}
